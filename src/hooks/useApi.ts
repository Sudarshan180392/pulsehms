...
export function useUnits() {
  return useQuery({
    queryKey: ['units'],
    queryFn: () => apiFetch<{ data: any[] }>(`${BASE}/v1/units`).then(r => r.data),
...
export function useCensus(unitId: string | null) {
  return useQuery({
    ...
    queryFn: () => apiFetch<{ beds: any[]; summary: any }>(`${BASE}/v1/units/${unitId}/census`),
    enabled: !!unitId,
...
export function usePatients(unitId: string | null) {
  return useQuery({
    ...
    queryFn: () => apiFetch<{ data: Patient[] }>(`${BASE}/v1/patients?unit_id=${unitId}&limit=500`).then(r => r.data),
    enabled: !!unitId,
    staleTime: 10_000,
  });
}
...
export function useAlerts(unitId: string | null) {
  return useQuery({
    ...
    queryFn: () => apiFetch<{ data: Alert[] }>(`${BASE}/v1/alerts?unit_id=${unitId}&status=active`).then(r => r.data),
    enabled: !!unitId,
...
export function useStaff(unitId: string | null) {
  return useQuery({
    ...
    queryFn: () => apiFetch<{ data: any[] }>(`${BASE}/v1/staff?unit_id=${unitId}`).then(r => r.data),
    enabled: !!unitId,
    staleTime: 30_000,
  });
}
...
export function useAdmitPatient() {
  ...
  return useMutation({
    mutationFn: async ({ patientId, etag, body }: { patientId: string; etag: string; body: object }) => {
      const res = await fetch(`${BASE}/v1/patients/${patientId}/admit`, {
        method: 'POST',
...
export function useDischargePatient() {
  ...
  return useMutation({
    mutationFn: async ({ patientId, etag, body }: { patientId: string; etag: string; body: object }) => {
      const res = await fetch(`${BASE}/v1/patients/${patientId}/discharge`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'If-Match': etag },
        body: JSON.stringify(body),
      });
...
export function useAcknowledgeAlert() {
  ...
  return useMutation({
    mutationFn: async ({ alertId, acknowledgedBy }: { alertId: string; acknowledgedBy: string }) => {
      ...
      const res = await fetch(`${BASE}/v1/alerts/${alertId}/acknowledge`, {
        method: 'POST',
